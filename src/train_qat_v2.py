...
Pipeline:
  1. Load or train the reduced float model (1433→16→24→7, root_weight=False)
...
FLOAT_MODEL_PATH = os.path.join(MODELS_DIR, "reduced_graphsage_no_root_best.pth")
...
def train_float_model(dataset, data, epochs=200, lr=0.01, dropout=0.5):
    print("\n" + "=" * 60)
    ...
    print(f"Saved to {FLOAT_MODEL_PATH}")
...
def main():
    ...
    if os.path.exists(FLOAT_MODEL_PATH):
        ...
        float_model = train_float_model(dataset, data, epochs=args.float_epochs)
    ...
    print(f"\nSaved QAT v2 model to {QAT_V2_MODEL_PATH}")
