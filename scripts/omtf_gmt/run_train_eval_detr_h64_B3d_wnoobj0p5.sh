#!/usr/bin/env bash
# DETR h64 B3d w_no_obj=0.5 — 100 epochs, cosine LR.
set -euo pipefail
ROOT_DIR="$(cd "$(dirname "${BASH_SOURCE[0]}")/../.." && pwd)"
cd "$ROOT_DIR"
PYTHON="$ROOT_DIR/venv/bin/python"
OUTDIR="build/omtf_gmt/checkpoints/detr_h64_B3d_wnoobj0p5_100ep"
...
for CKPT in \
    ...
    "$PYTHON" -u scripts/omtf_gmt/eval_gmt.py \
        ...
        --datasets   G1 G2 G3 G4 G5 G6 G7 G8 B4 \
        --threshold  0.0 \
        --output     "$EVALDIR/detr_h64_B3d_wnoobj0p5_100ep_${TAG}_eval.md" \
        --device     cuda
done
