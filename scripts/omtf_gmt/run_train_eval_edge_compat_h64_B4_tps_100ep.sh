#!/usr/bin/env bash
# EdgeCompat h64 — Phase B4 TPS: 100 epochs, cosine LR, periodic checkpoints.
# Mirrors run_train_eval_edge_compat_h64_B3d_100ep.sh for fair KMTF vs TPS comparison.
set -euo pipefail
ROOT_DIR="$(cd "$(dirname "${BASH_SOURCE[0]}")/../.." && pwd)"
cd "$ROOT_DIR"
PYTHON="$ROOT_DIR/venv/bin/python"
OUTDIR="build/omtf_gmt/checkpoints/edge_compat_h64_B4_tps_100ep"
EVALDIR="build/omtf_gmt/eval"

echo "=== TRAIN edge_compat h64 B4_tps 100ep ==="
...
for CKPT in \
    ...
    "$PYTHON" -u scripts/omtf_gmt/eval_gmt.py \
        ...
        --datasets   G1 G2 G3 G4 G5 G6 G7 G8 B4 \
        --threshold  0.0 \
        --output     "$EVALDIR/edge_compat_h64_B4_tps_100ep_${TAG}_eval.md" \
        --device     cuda
done
