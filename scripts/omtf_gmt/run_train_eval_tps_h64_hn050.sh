#!/usr/bin/env bash
# Run 2b — TPS-h64 + hard-negative candidate loss, w_hard_neg=0.50
set -euo pipefail
ROOT_DIR="$(cd "$(dirname "${BASH_SOURCE[0]}")/../.." && pwd)"
cd "$ROOT_DIR"
PYTHON="$ROOT_DIR/venv/bin/python"
OUTDIR="build/omtf_gmt/checkpoints/edge_compat_h64_tps_hn050"
EVALDIR="build/omtf_gmt/eval"

echo "=== TRAIN edge_compat h64 TPS hn=0.50 ==="
...
"$PYTHON" -u scripts/omtf_gmt/eval_gmt.py \
    ...
    --datasets   G1 G2 G3 G4 G5 G6 G7 G8 B4 \
    --threshold  0.0 \
    --output     "$EVALDIR/edge_compat_h64_tps_hn050_best_eval.md" \
    --device     cuda
