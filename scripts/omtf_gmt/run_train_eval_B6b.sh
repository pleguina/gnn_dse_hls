#!/usr/bin/env bash
# B6b — EdgeCompatAssign h64, w_assign=0.5, w_hard_neg=0.25
set -euo pipefail
ROOT_DIR="$(cd "$(dirname "${BASH_SOURCE[0]}")/../.." && pwd)"
cd "$ROOT_DIR"
PYTHON="$ROOT_DIR/venv/bin/python"

OUTDIR="build/omtf_gmt/checkpoints/edge_compat_assign_B6b"
EVALDIR="build/omtf_gmt/eval"

echo "=== TRAIN EdgeCompatAssign h64 B6b (w_assign=0.5) ==="
...
"$PYTHON" -u scripts/omtf_gmt/eval_gmt.py \
    ...
    --datasets   G1 G2 G3 G4 G5 G6 G7 G8 B4 \
    --threshold  0.0 \
    --output     "$EVALDIR/edge_compat_assign_B6b_best_eval.md" \
    --device     cuda
